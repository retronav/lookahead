...
//at beginning
func (f Firestore) GetDocument(path string) Document {
	return getDocument(path, f.IdToken)
}

//getCollection Fetch a document collection from Firestore.
//
//Path format:
//
//if collection is present at `/users/me/todos`, path should be `users/me/todos`
//without slash at beginning
func (f Firestore) GetCollection(path string) DocumentCollection {
	return getCollection(path, f.IdToken)
}
...
func shallowField(value jsonString) interface{} {
	...
	case gjson.Get(value, "booleanValue").Exists():
		...
		break
	case gjson.Get(value, "timestampValue").Exists():
		shallow, _ = time.Parse(time.RFC3339, gjson.Get(value, "timestampValue").Str)
		break
